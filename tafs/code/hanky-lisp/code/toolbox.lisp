...
(defun cut-list-by (item list &key (test #'eql))
  (labels ((clb (left out)
             (if left
                 (let ((now (car left)))
                   (if (funcall test item now)
                       (list (nreverse out) left)
                       (clb (cdr left) (push now out))))
                 (list list))))
    (clb list nil)))
'(docs "通过 item 切割列表并返回两个列表，如果切割失败第二个列表返回 nil")
'(example ((cut-list-by 3   '(1 2  3  4 5))
           (cut-list-by "3" '(1 2 "3" 4 5))
           (cut-list-by "3" '(1 2 "3" 4 5) :test #'equal)))
...
'(docs "使用 read-line 读取整个文件并将每一行作为列表一个元素，返回列表")

(defun load-string (input)
  (format nil "~{~A~}" (load-list-by #'read-char input)))
'(docs "使用 read-char 读取整个文件并将全部作为一个字符串并返回")
...
(defun get-now-time (&optional (time-format '("-" "-" "/" ":" ":" "")))
  (multiple-value-bind (second minute hour day month year)
      (decode-universal-time (get-universal-time))
    (format nil "~A~A~2,'0d~A~2,'0d~A~2,'0d~A~2,'0d~A~2,'0d~A"
            year   (nth 0 time-format)
            month  (nth 1 time-format)
            day    (nth 2 time-format)
            hour   (nth 3 time-format)
            minute (nth 4 time-format)
            second (nth 5 time-format))))
'(docs "获取当前时间")
'(example ((get-now-time)
           (get-now-time '("/" "/" " " ":" ":" ""))
           (progn
             (dotimes (i 5)
               (format t "~A~%" (get-now-time))
               (sleep 1))
             (format t "~A~%" (get-now-time)))))
